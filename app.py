...
def handle_ask(question):
    ...
    if used >= MAX_GUEST_QUESTIONS:
        return gr.update(value=""), gr.update(value="❌ Guest users can only ask up to 5 questions. Please sign up to continue.", visible=True), gr.Tabs.update(selected=0)
    ...
    return gr.update(value=answer + f"\n\n📌 You have {remaining}/5 questions remaining as a guest."), gr.update(visible=False), gr.Tabs.update()
...
with gr.Blocks() as interface:
    ...
    with gr.Tabs() as tabs:
        with gr.Tab("Login", id=0):
            ...
            signup_user = gr.Textbox(label="Username (Signup)")
            signup_email = gr.Textbox(label="Email Address")
            signup_pass = gr.Textbox(label="Password (Signup)", type="password")
